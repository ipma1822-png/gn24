/* Shared country-edition renderer. Country identity and content are injected per edition. */
(() => {
  'use strict';
  ...
  const home = () => {intro(config.editionName,ui.intro);const lead=articles.find(a=>a.featured)||articles[0];if(!lead)return;const layout=text('div','','global-edition-lead-grid');const featured=link('',storyHref(lead),'global-edition-lead');featured.append(media(lead));const copy=text('div','','global-edition-lead-copy');copy.append(text('span',ui.top+' · '+badge(lead),'global-edition-meta'),text('h2',lead.title),text('p',lead.summary));featured.append(copy);const side=text('aside','','global-edition-side');side.append(text('h2',ui.latest));articles.filter(a=>a!==lead).slice(0,4).forEach(a=>{const item=link('',storyHref(a));item.append(text('span',a.title),text('small',formatDate(a.date)));side.append(item)});layout.append(featured,side);main.append(layout);section(ui.main,articles.filter(a=>a!==lead),'main-news');section(ui.local,articles.filter(a=>a.scope==='country-local'),'local-news');section(ui.global,articles.filter(a=>a.scope==='global-feed'),'global-news');[['martial','martial'],['safety','safety'],['technology','technology'],['cooperation','cooperation']].forEach(([label,key])=>section(ui[label],articles.filter(a=>a.category===key),key));};
  const newsroom = () => {intro(ui.newsroom,ui.intro);const form=text('form','','global-edition-search');form.action=base;form.method='get';const hidden=document.createElement('input');hidden.type='hidden';hidden.name='view';hidden.value='newsroom';form.append(hidden);const input=document.createElement('input');input.type='search';input.name='q';input.value=params.get('q')||'';input.placeholder=ui.search;input.setAttribute('aria-label',ui.search);const submit=text('button',ui.searchButton);submit.type='submit';form.append(input,submit);main.append(form);const filters=text('nav','','global-edition-filters');filters.setAttribute('aria-label',ui.newsroom);[['',ui.all],['local',ui.local],['global',ui.global],['martial',ui.martial],['safety',ui.safety],['technology',ui.technology],['cooperation',ui.cooperation]].forEach(([key,label])=>{const item=link(label,pageUrl('newsroom',{cat:key}),'global-edition-filter');if((params.get('cat')||'')===key)item.setAttribute('aria-current','page');filters.append(item)});main.append(filters);const q=(params.get('q')||'').trim().toLocaleLowerCase(config.locale);const selected=params.get('cat')||'';const list=articles.filter(a=>(!selected||a.category===selected)&&(!q||[a.title,a.summary,a.category].join(' ').toLocaleLowerCase(config.locale).includes(q)));const results=text('div','','global-edition-news-list');if(!list.length)results.append(text('p',ui.noResults,'global-edition-empty'));else list.forEach(a=>results.append(row(a)));main.append(results);};
