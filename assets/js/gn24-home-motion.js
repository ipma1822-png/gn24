...
function ranking(){
 ...
 const size=()=>requestAnimationFrame(()=>{
   let total=0;
   [...track.children].forEach(el=>{const cs=getComputedStyle(el);total+=el.getBoundingClientRect().height+(parseFloat(cs.marginTop)||0)+(parseFloat(cs.marginBottom)||0);});
 ...
 function move(){
  ...
  track.style.transition='transform 1.25s ease-in-out'; track.style.transform=`translate3d(0,-${d}px,0)`;
  setTimeout(()=>{track.style.transition='none';track.appendChild(first);track.style.transform='translate3d(0,0,0)';void track.offsetHeight;busy=false;},1300);
 }
...
function editor(){
 ...
 function move(){
  ...
  const first=grid.firstElementChild,gap=parseFloat(getComputedStyle(grid).gap)||16,step=first.getBoundingClientRect().width+gap;
  grid.style.transition='transform 1.35s ease-in-out';grid.style.transform=`translate3d(-${step}px,0,0)`;
  setTimeout(()=>{grid.style.transition='none';grid.appendChild(first);grid.style.transform='translate3d(0,0,0)';void grid.offsetWidth;busy=false;},1400);
 }
