...
const DEFAULT_IMAGE='/assets/images/news/gn24-default-news.svg',LOCAL_TARGET=12;
...
async function loadNews(){try{
 ...
 const q=new URLSearchParams(location.search),cat=q.get('cat');
 const localView=cat?local.filter(a=>a.category===cat):local;
 const hqView=cat?hq.filter(a=>a.category===cat):hq;
 // Early-stage regional newsroom: local stories always lead; empty slots are filled by HQ stories without cloning article IDs.
 const blended=[...localView,...hqView.filter(a=>!localView.some(l=>l.id===a.id))];
 $('#regionalCount')&&($('#regionalCount').textContent=local.length<LOCAL_TARGET?`${local.length}건 · 본사뉴스 함께보기`:`${localView.length}건`);
