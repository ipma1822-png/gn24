(() => {
  ...
  const STORE = {
    ...
    anonKey: window.NEWSNA_HAND_WIZARD_ANON_KEY || ''
  };
  const esc = (v='') => String(v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
  const safePhoto = v => /^data:image\/(jpeg|png|webp);base64,/i.test(v || '') ? v : '';

  async function loadCard() {
    if (!STORE.anonKey) return null;
    const r = await fetch(`${STORE.url}/rest/v1/rpc/get_hand_wizard_card`, {
      method:'POST',
      headers:{apikey:STORE.anonKey,Authorization:`Bearer ${STORE.anonKey}`,'Content-Type':'application/json'},
  ...
  function render(card) {
    if (!card?.name) return;
    const name=String(card.name).trim().slice(0,40), honor=String(card.honor||'님').trim().slice(0,20), message=String(card.message||'').trim().slice(0,500);
    const chat=document.getElementById('chat'), ashead=document.querySelector('.ashead'), assistant=document.querySelector('.assistant');
    ...
    const el=document.createElement('section'); el.className='newsna-personal-card';
    const photo=safePhoto(card.photo);
    el.innerHTML=`${photo?`<img class="newsna-personal-photo" src="${photo}" alt="${esc(name)}님 개인 사진">`:'<div class="newsna-personal-avatar">N</div>'}<div><small>손안의 마법사 · 개인 초대</small><b>${esc(name)} ${esc(honor)}께</b>${message?`<p>${esc(message)}</p>`:''}</div>`;
  ...
  window.NewsnaPersonalization={id,loadCard,render};
  loadCard().then(render).catch(e=>console.warn('[NEWSNA PERSONALIZATION]',e));
})();